...
namespace ModiBuff.Tests
{
	public sealed class MetaEffectTests : ModifierTests
	{
		...
		public void CanCastHalfMulti_IsStunnedDoubleMulti()
		{
			...
			Enemy.StatusEffectController.ChangeStatusEffect(0, 0, StatusEffectType.Disarm, 1f);

			Unit.TryCast(generator.Id, Enemy); //5 * 2f
			Assert.AreEqual(EnemyHealth - 5 - 10f, Enemy.Health);

			Enemy.Update(1f);
			Enemy.StatusEffectController.ChangeStatusEffect(0, 0, StatusEffectType.Silence, 1f);
		...
		public void DoubleMultiplierWhenSilenced()
		{
			...
			Unit.StatusEffectController.ChangeStatusEffect(0, 0, StatusEffectType.Disarm, 1f);
