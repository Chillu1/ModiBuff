...
namespace ModiBuff.Examples.SimpleSolo
{
	...
	/// </summary>
	public sealed class Unit : IUnit, IUpdatable, IModifierOwner, IAttacker<float, float>, IDamagable<float, float>, IBlockOwner
	{
		...
		{
			float dealtDamage = ((IDamagable<float, float, float, float>)target).TakeDamage(Damage, this, triggersEvents);
